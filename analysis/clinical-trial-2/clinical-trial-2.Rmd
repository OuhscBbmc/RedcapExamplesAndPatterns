...
read_chunk("./analysis/clinical-trial-2/clinical-trial-2.R") #This allows knitr to call chunks tagged in the underlying *.R file.
```
...
## Notes 
1. The current report covers `r nrow(ds)` patients, with `r dplyr::n_distinct(ds$race)` unique values for `race`.
...
## Gender & Ethnicity
```{r table-gender-ethnicity, echo=echo_chunks, message=message_chunks}
```

## Gender & Race
```{r table-gender-race, echo=echo_chunks, message=message_chunks}
```

# Graphs

## Marginals
```{r marginals, echo=echo_chunks, message=message_chunks, fig.width=7}
```
